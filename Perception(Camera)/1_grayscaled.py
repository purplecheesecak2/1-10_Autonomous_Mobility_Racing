...
class WebcamGrayscale(Node):
    def __init__(self):
        ...
        self.get_logger().info("웹캠 Grayscale 노드가 시작되었습니다.")
        self.get_logger().info("=" * 50)
        self.get_logger().info("📹 컬러 이미지 토픽: /camera/color/image_raw")
        self.get_logger().info("⚫ Grayscale 이미지 토픽: /camera/gray/image_raw")
        self.get_logger().info(f"📐 해상도: {actual_width}x{actual_height}")
        self.get_logger().info(f"🎬 FPS: {actual_fps}")
        self.get_logger().info("🔧 백엔드: V4L2")
        self.get_logger().info("=" * 50)
        self.get_logger().info("🖼️  이미지 보기:")
        self.get_logger().info("   ros2 run rqt_image_view rqt_image_view /camera/gray/image_raw")
        self.get_logger().info("=" * 50)
    ...
    def timer_callback(self):
        ...
        try:
            # 원본 컬러 이미지를 ROS2 메시지로 변환 후 publish
