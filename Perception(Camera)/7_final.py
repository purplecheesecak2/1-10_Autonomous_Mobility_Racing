...
class LaneDetectionFinal(Node):
    ...
    def timer_callback(self):
        ...
        try:
            ...
            # 4. Canny Edge (첫 번째 코드용)
            edges_old = cv2.Canny(roi, self.canny_threshold1, self.canny_threshold2)
            
            # 5. BEV (첫 번째 코드용)
            bev_old = cv2.warpPerspective(edges_old, self.perspective_matrix,
            ...
            # 3. ROI 적용
            white_mask_roi = white_mask[self.roi_start_height:self.img_height, 0:self.img_width]
            yellow_mask_roi = yellow_mask[self.roi_start_height:self.img_height, 0:self.img_width]
            
            # 4. Noise Reduction (흰색 마스크에)
            blur_white = cv2.GaussianBlur(white_mask_roi, (self.gaussian_kernel_size, self.gaussian_kernel_size), 0)
            
            # 5. Canny Edge (흰색만)
            edges_new = cv2.Canny(blur_white, self.canny_threshold1, self.canny_threshold2)
            
            # 6. BEV (두 번째 코드용)
            bev_new = cv2.warpPerspective(edges_new, self.perspective_matrix,
                                         (self.img_width, self.roi_height))
            
            # 7. Sliding Window로 차선 픽셀 찾기
            leftx, lefty, rightx, righty, out_img = self.find_lane_pixels(bev_new)
            
            # 8. Polynomial Fit
            left_fitx, right_fitx, ploty = self.fit_polynomial(bev_new, leftx, lefty, rightx, righty)
            ...
            # 노란 벽을 빨간색으로 오버레이
            yellow_bev = cv2.warpPerspective(yellow_mask_roi, self.perspective_matrix,
                                             (self.img_width, self.roi_height))
            result[yellow_bev > 0] = [0, 0, 255]
            
            # ====== OpenCV 창에 표시 (3개 창) ======
            
            # 1번 창: ROI + Canny Edge (첫 번째 코드 방식)
            roi_display = cv2.cvtColor(roi, cv2.COLOR_GRAY2BGR)
            edges_display = cv2.cvtColor(edges_old, cv2.COLOR_GRAY2BGR)
            cv2.imshow('1. ROI + Canny Edge', edges_display)
            
            # 2번 창: Birds-Eye View (첫 번째 코드 방식)
            bev_display = cv2.cvtColor(bev_old, cv2.COLOR_GRAY2BGR)
            ...
            # 3번 창: Lane Detection (두 번째 코드 방식)
            ...
            bev_msg = self.bridge.cv2_to_imgmsg(bev_new, encoding="mono8")
