...
class LaneDetectionFinal(Node):
    def __init__(self):
        ...
        self.cam_x     =  0.06    # 카메라 전후 위치 m (차량 뒤쪽이면 음수, 앞쪽이면 양수)
        self.cam_z     =  0.235   # 카메라 높이 m
        self.cam_pitch = -0.4   # 카메라 pitch 각도 deg (아래를 향하면 음수)
        # 픽셀 → 미터 스케일 (바닥 캘리브레이션 후 조정)
        self.scale_x   =  240    # 카메라 전방 시야 거리 m (ROI 세로 = 이 거리에 해당)
        self.scale_y   =  3.2    # 카메라 좌우 시야 거리 m (화면 가로 = 이 거리에 해당)
        # ──────────────────────────────────────────────────────────────────────
    ...
    def _yellow_x_bounds(self, yellow):
        """노란선 마스크 하단 25%만 보고 왼쪽/오른쪽 경계 x를 계산.
        하단만 보는 이유: 코너에서 노란선이 ROI 상단에서 화면 중앙으로 넘어올 때
        x_min이 너무 오른쪽으로 잡혀 안쪽 차선을 잘라버리는 문제 방지.
        보이면 즉시 갱신, 안 보이면 0(왼쪽) 또는 W(오른쪽)으로 리셋."""
        h   = yellow.shape[0]
        mid = self.W // 2
        # 하단 25%만 사용 (코너 노란선이 중앙으로 넘어오는 상단 영역 제외)
    ...
    def timer_callback(self):
        ...
        try:
            ...
            # ROI 하단 40% 이내에 컨투어 하단이 있어야 유효 (상단 노이즈 차단)
            y_threshold = int(roi_h * 0.60)
            ...
            for c in cnts:
                ...
                pts_x = c[:, 0, 0]
                pts_y = c[:, 0, 1]
                # y축: 컨투어 하단이 ROI 하단 40% 안에 없으면 상단 노이즈로 무시
                if pts_y.max() < y_threshold: continue
                # x축: 컨투어 x 범위의 중앙값이 노란선 경계 안에 있어야 통과
                x_median = int(np.median(pts_x))
                if x_median < x_min or x_median > x_max: continue
                # 컨투어 픽셀의 70% 이상이 x_min~x_max 범위 안에 있어야 통과
                in_range = np.sum((pts_x >= x_min) & (pts_x <= x_max))
                if in_range / len(pts_x) < 0.7: continue
