...
class LaneDetectionFinal(Node):
    ...
    def _yellow_x_bounds(self, yellow):
        """노란선 마스크에서 하단 절반 기준 좌측/우측 x 범위를 반환.
        노란선이 없으면 (0, W) 반환 — 전체 허용"""
        h = yellow.shape[0]
        bottom_half = yellow[h//2:, :]
        cols = np.where(bottom_half.any(axis=0))[0]
        if len(cols) == 0:
            return 0, self.W
        # 왼쪽 노란선: cols 중 왼쪽 절반, 오른쪽 노란선: 오른쪽 절반
        mid = self.W // 2
        left_cols  = cols[cols < mid]
        right_cols = cols[cols >= mid]
        x_min = int(left_cols.max())  if len(left_cols)  > 0 else 0
        x_max = int(right_cols.min()) if len(right_cols) > 0 else self.W
        return x_min, x_max
    ...
    def timer_callback(self):
        ...
        try:
            ...
            # ★ 노란선 x 범위 계산 → 흰선은 이 범위 안에 있어야 유효
            ...
            # cx가 어느 contour 안에 있는지 pointPolygonTest로 판정
            ...
            if current_i >= 0:
                self.current_lane_x = lanes[current_i][0]
                offset = (self.current_lane_x - cx) / cx
            else:
                offset = 0.0
            ...
            # HUD
            in_lane_lbl = f"lane {current_i}" if current_i >= 0 else "---"
