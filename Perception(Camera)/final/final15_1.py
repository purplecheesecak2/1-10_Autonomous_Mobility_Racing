...
class LaneDetectionFinal(Node):
    ...
    def timer_callback(self):
        ...
        try:
            ...
            clear_marker.ns = "lanes"
            clear_marker.action = Marker.DELETEALL
            marker_array.markers.append(clear_marker)

            # 노란선 경계 마커 (초록 세로선 2개)
            for idx_b, bx in enumerate([x_min, x_max]):
                bm = Marker()
                bm.header.frame_id = "camera"
                ...
                bm.scale.x = 0.003
                bm.color.r, bm.color.g, bm.color.b, bm.color.a = 0.0, 0.7, 0.0, 1.0
                for fy in [0.0, 1.0]:
                    p = Point()
                    p.x = (bx - self.W / 2) / self.W
                    p.y = 0.5 - fy
                    p.z = 0.0
                    bm.points.append(p)
            ...
            for i, (bot_x, c) in enumerate(lanes):
                m_lane = Marker()
                m_lane.header.frame_id = "camera"
                m_lane.header.stamp = stamp
                m_lane.ns = "lanes"
                m_lane.id = i + 1
                m_lane.type = Marker.LINE_STRIP
                m_lane.action = Marker.ADD
                m_lane.scale.x = 0.006

                if i == current_i:       # 현재 차선: 초록
                    m_lane.color.r, m_lane.color.g, m_lane.color.b, m_lane.color.a = 0.0, 1.0, 0.0, 1.0
                else:                    # 다른 차선: 노란
                    m_lane.color.r, m_lane.color.g, m_lane.color.b, m_lane.color.a = 1.0, 1.0, 0.0, 1.0

                step = max(1, len(c) // 30)
                for pt in c[::step]:
                    p = Point()
                    p.x = (pt[0][0] - self.W / 2) / self.W
                    p.y = -(pt[0][1] - roi_h / 2) / roi_h
                    p.z = 0.0
                    m_lane.points.append(p)

                marker_array.markers.append(m_lane)
            ...
            if current_i >= 0:
                ...
                cm.scale.x = cm.scale.y = cm.scale.z = 0.03
                cm.color.r, cm.color.g, cm.color.b, cm.color.a = 0.0, 1.0, 1.0, 1.0
                cm.pose.position.x = (self.current_lane_x - self.W / 2) / self.W
                cm.pose.position.y = -0.4
