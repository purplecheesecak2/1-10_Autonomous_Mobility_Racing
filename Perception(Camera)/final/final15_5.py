...
class LaneDetectionFinal(Node):
    def __init__(self):
        ...
        self.path_left_pub    = self.create_publisher(Path,   "/lane/path/left",      1)
        self.path_center_pub  = self.create_publisher(Path,   "/lane/path/center",    1)
        self.path_right_pub   = self.create_publisher(Path,   "/lane/path/right",     1)
    ...
    def _broadcast_camera_tf(self):
        ...
        t.transform.translation.x = 0.0   # 차량 전후 위치 (실측 후 조정, 뒤쪽이면 음수)
        t.transform.translation.y = 0.0   # 좌우 중앙
        t.transform.translation.z = 0.30  # 높이 30cm
        # 회전: 카메라가 전방을 향하고 약간 아래를 봄 (pitch -15도 정도)
    ...
    def timer_callback(self):
        ...
        try:
            ...
            def contour_to_path(contour, frame_id='camera', deg=2, n_out=30):
                ...
                if not smooth_pts:
                    return path
                # 픽셀 → 미터 스케일 (실측 후 조정 필요)
                scale_x = 3.0 / roi_h   # 세로 픽셀 → 전방 거리(m)
                scale_y = 2.0 / self.W  # 가로 픽셀 → 좌우 거리(m)
            ...
            # 현재 차선 기준 양옆 포함 최대 3개 Path 퍼블리시
            # left: current_i-1, center: current_i, right: current_i+1
            empty_path = Path(); empty_path.header.stamp = stamp; empty_path.header.frame_id = 'camera'

            if current_i >= 0:
                # center (내 차선)
                self.path_center_pub.publish(contour_to_path(lanes[current_i][1]))
                # left: 흰 차선이면 흰 차선 좌표, 노란선이면 노란선 좌표
                if current_i > 0:
                    self.path_left_pub.publish(contour_to_path(lanes[current_i - 1][1]))
                elif left_yel:
                    self.path_left_pub.publish(contour_to_path(max(left_yel, key=cv2.contourArea)))
                else:
                    self.path_left_pub.publish(empty_path)
                # right: 흰 차선이면 흰 차선 좌표, 노란선이면 노란선 좌표
                if current_i < len(lanes) - 1:
                    self.path_right_pub.publish(contour_to_path(lanes[current_i + 1][1]))
                elif right_yel:
                    self.path_right_pub.publish(contour_to_path(max(right_yel, key=cv2.contourArea)))
                else:
