...
class LaneDetectionFinal(Node):
    ...
    # ────────────────────────────────────────────
    def _extract_peaks(self, hist, min_val=150, min_dist=80, max_peaks=2):
        peaks = []
        tmp = hist.copy()
    ...
    def _find_peaks(self, white, yellow):
        ...
        # 노란선: min_dist 50으로 줄여서 양쪽 벽 다 잡기
        y_peaks = self._extract_peaks(hy, min_val=150, min_dist=50, max_peaks=2)

        # 흰선: 노란선 위치에서 60px 이내 피크는 제거 (노란선 위 오검출 방지)
        hw_filtered = hw.copy()
        for yp in y_peaks:
            hw_filtered[max(0, yp-60):min(len(hw_filtered), yp+60)] = 0
        w_peaks = self._extract_peaks(hw_filtered, min_val=150, min_dist=80, max_peaks=2)
