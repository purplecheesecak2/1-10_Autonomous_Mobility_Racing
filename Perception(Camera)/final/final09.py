...
class LaneDetectionFinal(Node):
    ...
    def _find_peaks(self, white, yellow):
        ...
        # 노란선: 매 프레임 히스토그램에서 직접 좌/우 분리 (prev 의존 최소화)
        left_y  = next((p for p in sorted(raw_y_peaks) if p < self.W // 2), None)
        right_y = next((p for p in sorted(raw_y_peaks, reverse=True) if p >= self.W // 2), None)

        # 안정화: 이전값에서 80px 이상 튀면 이전값 유지
        for i, val in enumerate([left_y, right_y]):
            if val is not None:
                if self.y_peak_prev[i] is not None and abs(val - self.y_peak_prev[i]) > 80:
                    if i == 0: left_y  = self.y_peak_prev[0]
                    else:      right_y = self.y_peak_prev[1]
                else:
                    self.y_peak_prev[i] = val
            # 이번 프레임 못 찾으면 이전값 유지
            elif self.y_peak_prev[i] is not None:
                if i == 0: left_y  = self.y_peak_prev[0]
                else:      right_y = self.y_peak_prev[1]

        y_peaks = sorted([p for p in [left_y, right_y] if p is not None])
        ...
        hw[right_bound:] = 0
        for yp in y_peaks:
            hw[max(0, yp-60):min(len(hw), yp+60)] = 0
        w_peaks = self._extract_peaks(hw, min_val=100, min_dist=80, max_peaks=3)
    ...
    def timer_callback(self):
        ...
        try:
            ...
            # 노란선: x 위치 기준 왼쪽/오른쪽 분리 후 fit 저장
            left_y_peak  = next((p for p in y_peaks if p < self.W // 2), None)
            right_y_peak = next((p for p in reversed(y_peaks) if p >= self.W // 2), None)
            self._update_fit('y0', yellow, left_y_peak,  out_img, (0,0,255))
            self._update_fit('y1', yellow, right_y_peak, out_img, (0,0,255))

            # 노란선 clip용: fit보다 peak 수직선이 더 안정적
            def lane_x_arr(fit, peak, py):
                if peak is not None:
                    # fit이 있어도 하단 x가 peak과 너무 다르면 수직선 사용
                    if fit is not None:
                        bot_x = fit[0]*py[-1]**2 + fit[1]*py[-1] + fit[2]
                        if abs(bot_x - peak) < 80:
                            return (fit[0]*py**2 + fit[1]*py + fit[2]).astype(np.int32)
                    return np.full(len(py), peak, dtype=np.int32)
                return None

            ly_arr = lane_x_arr(self.fit_prev['y0'], left_y_peak, ploty)
            ry_arr = lane_x_arr(self.fit_prev['y1'], right_y_peak, ploty)

            # 디버그: ry_arr 하단값 표시
            if ry_arr is not None:
                cv2.putText(out_img, f"ry_bot:{ry_arr[-1]}", (10,86),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,200,255), 1)
            if ly_arr is not None:
                cv2.putText(out_img, f"ly_bot:{ly_arr[-1]}", (10,104),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,200,255), 1)

            # 흰선 (sliding window는 경계 없이, 그릴 때 노란선으로 clip)
            ...
            for key, lbl in [('y0','L'), ('y1','R')]:
                ...
                mid = len(ploty) // 2
                cv2.putText(out_img, lbl, (int(x[mid]), int(ploty[mid])),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (235, 206, 135), 2)

            # 흰선 곡선 (ly_arr/ry_arr로 직접 clip)
            for fx, key in w_fits:
                fit   = self.fit_prev[key]
                color = (0,255,0) if key == current_key else (0,255,255)
                x = (fit[0]*ploty**2 + fit[1]*ploty + fit[2]).astype(np.int32)
                if ly_arr is not None:
                    x = np.maximum(x, ly_arr)
                if ry_arr is not None:
                    x = np.minimum(x, ry_arr)
